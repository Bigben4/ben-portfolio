...
function App() {
  return <main>
    ...
    <section className="skills"><SectionTitle eyebrow="TECH STACK" title="Technologies I Work With" /><div className="skill-row"><button>←</button>{skills.map(([icon,name]) => <article key={name}><b>{icon}</b><span>{name}</span></article>)}<button>→</button></div><div className="dots">● • • •</div></section>
