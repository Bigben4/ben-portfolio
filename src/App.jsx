...
function App() {
  ...
  useEffect(() => {
    setSkillPage((page) => Math.min(page, skillPages.length - 1))
  }, [skillPages.length])
  ...
  return <main>
    ...
    <section className="skills"><SectionTitle eyebrow="TECH STACK" title="Technologies I Work With" /><div className="skills-carousel" aria-roledescription="carousel" aria-label="Technologies I work with"><button className="carousel-control" type="button" aria-label="Previous technologies" onClick={() => setSkillPage((page) => Math.max(0, page - 1))} disabled={skillPage === 0}>←</button><div className="skills-viewport"><div className="skills-track" style={{ transform: `translateX(-${skillPage * 100}%)` }}>{skillPages.map((page, pageIndex) => <div className="skill-page" key={pageIndex} aria-hidden={pageIndex !== skillPage}>{page.map(([icon, name]) => <article key={name}><b>{icon}</b><span>{name}</span></article>)}</div>)}</div></div><button className="carousel-control" type="button" aria-label="Next technologies" onClick={() => setSkillPage((page) => Math.min(skillPages.length - 1, page + 1))} disabled={skillPage === skillPages.length - 1}>→</button></div><div className="dots" role="tablist" aria-label="Technology carousel pages">{skillPages.map((_, page) => <button key={page} type="button" role="tab" aria-label={`Show technology page ${page + 1}`} aria-selected={skillPage === page} onClick={() => setSkillPage(page)} />)}</div></section>
